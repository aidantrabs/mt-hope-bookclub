...
const buildSubmitData = (f: FormData) => ({
    ...
    dateFinalDiscussion: f.dateFinalDiscussion,
    summary: f.summary,
    ratingClub: f.ratingClub,
    ratingGoodreads: f.ratingGoodreads,
        ...
        setForm((current) => {
            ...
            if (result.success) {
                ...
                const fieldError = result.error.issues.find(
                    (issue) => issue.path[0] === key || issue.path.join(".") === key,
                );
                if (fieldError) {
                    ...
                    <div className="bg-bg-card rounded-xl p-6 max-w-sm w-full shadow-lg">
                        ...
                        <div className="flex gap-3 justify-end">
                            <button
                                ...
                                className="px-4 py-2 rounded-lg bg-border text-text-primary hover:bg-border/80 transition-colors font-medium text-sm"
                            >
                                stay
                            </button>
                            <button
            ...
            <form onSubmit={handleSubmit} className="space-y-10">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <Field label="date started" error={errors.dateStarted} field="dateStarted">
                            <input
                                ...
                                className={inputClass(errors.dateStarted)}
                            />
                        </Field>
                        <Field label="final discussion" error={errors.dateFinalDiscussion} field="dateFinalDiscussion">
                            <input
                                type="date"
                                value={form.dateFinalDiscussion}
                                onChange={(e) => update("dateFinalDiscussion", e.target.value)}
                                onBlur={() => {
                                    if (
                                        form.dateStarted &&
                                        form.dateFinalDiscussion &&
                                        form.dateFinalDiscussion < form.dateStarted
                                    )
                                        setErrors((prev) => ({
                                            ...prev,
                                            dateFinalDiscussion: "must be on or after the start date",
                                        }));
                                }}
                ...
                <fieldset className="space-y-4">
                    ...
                    <Field label="summary" error={errors.summary} field="summary">
                        <textarea
                            ...
                            placeholder="a brief summary of the book and how the club felt about it..."
                            rows={3}
                    ...
                    <Field label="fun fact" error={errors.funFact} field="funFact">
                        <textarea
                            ...
                            onBlur={() => validateField("funFact")}
                            onInput={(e) => {
                                const t = e.target as HTMLTextAreaElement;
                                t.style.height = "auto";
                                t.style.height = `${t.scrollHeight}px`;
                            }}
                            placeholder="something interesting about the book or author..."
                            rows={2}
                            className={inputClass(errors.funFact)}
                        />
                    </Field>
                </fieldset>
                ...
                <fieldset className="space-y-4">
                    ...
                    <ListField
                        ...
                        error={errors.discussionHighlights}
                        field="discussionHighlights"
                    />
                </fieldset>
                ...
                <fieldset className="space-y-4">
                    ...
                    <Field label="next read" error={errors.nextRead} field="nextRead">
                        <input
                            type="text"
                            value={form.nextRead}
                            onChange={(e) => update("nextRead", e.target.value)}
                            placeholder="title of the next book (optional)"
                            className={inputClass(errors.nextRead)}
                        />
                    </Field>
                        ...
                        <div className="flex gap-2">
                            ...
                            <button
                                ...
                                onClick={async () => {
                                    ...
                                    const r = await searchBookCover(form.title);
                                    if (r) {
                                        update("coverImageUrl", r.coverUrl);
                ...
                <div key={i} className="flex gap-2">
                    ...
                    {items.length > 1 && (
                        <button
                            ...
                            className="text-text-secondary hover:text-red-600 transition-colors px-2 shrink-0"
                            aria-label="remove"
                        >
                            ✕
                        </button>
